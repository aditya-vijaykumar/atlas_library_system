...
contract Ownable {
  ...
  event PrimaryChanged(address previousPrimary, address newPrimary);

  // Owner of the contract
  address private _owner;
  //Transactor to the contract
 ...
 constructor() public {
  ...
  function setPrimary(address newPrimary) public onlyOwner {
    emit PrimaryChanged(_primary, newPrimary);
    _primary = newPrimary;
  }
...
contract TokenVigil_v1_4 is ERC20 {

	mapping (address => bool) internal _signedup;
    ...
    function signedUp(address owner) public view returns (bool) {
        return _signedup[owner];

    }


    function signup(address account, uint256 value) public onlyOwner {
        require (!_signedup[account]);
        mint(account, value);
        _signedup[account] = true;
    }
